...
def get_symm_system(conn, debug):
    ...
    """
    systems = conn.EnumerateInstances('Symm_StorageSystem', namespace='root/emc')
    local_sys = ""
    ...
    time_diff_in_minutes = divmod(time_diff.days * 86400 + time_diff.seconds, 60)[0]
    if time_diff_in_minutes > 15:
        print("***** The SMI-S VMAX array data is stale and requires refreshing. The last sync was "
              + str(time_diff_in_minutes) + " minutes ago. The data syncs automatically only "
...
def get_rep_volumes(conn, symm, luns, debug):
...
def find_existing_pairs(conn, symm, luns, source_luns, debug):
    ...
    for lun in rep_targets:
        if lun["EMCWWN"] in luns:
            pairs = conn.Associators(lun.path, ResultClass="SE_ReplicaPairView")
            for pair in pairs:
                if pair["SVTargetDeviceID"] == lun["DeviceId"]:
                    ...
                    for source_lun in source_luns:
                        if source_lun["DeviceID"] == pair["SVSourceDeviceID"]:
                            rep_pairs[pair["SVSourceDeviceID"]] = pair["SVTargetDeviceID"]
...
def print_xml(symm, rep_pairs, debug):
    ...
    version = ET.SubElement(root, "Version").text= "6.2"
    copy_info = ET.SubElement(root, "CopyInfo")
    array_id = ET.SubElement(copy_info, "ArrayId").text= symm[12:]
    copy_type = ET.SubElement(copy_info, "CopyType").text= COPY_TYPE
    copy_mode = ET.SubElement(copy_info, "CopyMode").text= COPY_MODE
    save_pool_name = ET.SubElement(copy_info, "SavePoolName")
...
def get_symm_conn(debug):
    ...
    """
    conn = pywbem.WBEMConnection(SMIS_IP+":"+SMIS_PORT, (SMIS_USER, SMIS_PASS))
    return conn
...
def main():
    ...
    """
    debug = False
    parser = argparse.ArgumentParser(description='Create failover pairing XML file.')
    ...
    luns = get_rep_volumes(symm_conn, symm, vsphere_content["luns"], debug)
    filter_used_luns(symm, vsphere_content["extents"], luns["target"], debug)
    rep_pairs = find_existing_pairs(symm_conn, symm, vsphere_content["luns"], luns["source"], debug)
